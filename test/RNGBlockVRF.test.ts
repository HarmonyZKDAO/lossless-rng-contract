...
const debug = require('debug')('ptv3:RNGBlockVRF.test');

describe('RNGBlockVRF contract', function () {
  ...
  beforeEach(async () => {
    ...
    debug('Deploying RNG...');
    const RNGFactory = await getContractFactory('RNGBlockVRFHarness', deployer);
  ...
  describe('randomNumber()', () => {
    it('should return a previous random number by request ID', async () => {
      ...
      const randomNumber = await rng.randomNumber(requestId);
